...
async function getUpgradesForBuy() {
    try {
        const response = await axios({
            url: env.URL + "/clicker/upgrades-for-buy",
            method: "POST",
            headers: headers
        });
...
async function main() {
    while (true) {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0.01 * 60 * 1000)); // 5 минут * 60 секунд * 1000 миллисекунд
    }
}
