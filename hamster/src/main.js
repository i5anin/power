...
async function getBalance() {
  try {
    ...
    if (error.response && error.response.status === 403) {
      console.error(
        chalk.red('Ошибка при обновлении баланса: ' + error.response.data.message)
      )
    } else {
      console.error('Ошибка при обновлении баланса:', error)
    }
...
function logBalanceInfo(clickerUser) {
  const { balanceCoins: balance, earnPassivePerSec, earnPassivePerHour } =
    clickerUser
  ...
  console.log(
    ...
    `• Прирост: ${chalk.yellow(
      earnPassivePerHour.toLocaleString()
    )} в час ` +
...
function calculateTimeToBuy(clickerUser, nearestUpgrade) {
  ...
  const secondsToBuy = Math.ceil(
    priceDifference / clickerUser.earnPassivePerSec
  )
        ...
        if (affordableUpgrades.length > 0) {
          ...
          console.log(
            ...
            `${bestUpgrade.section}: ${
              ...
              bestUpgrade.price.toLocaleString()
            )} ` +
            `- окупаемость: ${chalk.blue(
              bestUpgrade.paybackPeriod.toFixed(2)
            )} ч.`
          )
          ...
          try {
            ...
            if (buyResult.clickerUser && buyResult.clickerUser.upgrades) {
              ...
              console.log(
                chalk.red(`Ошибка: Недостаточно средств для покупки.`)
              )
              affordableUpgrades.shift() // Удаляем апгрейд из списка доступных
            } else {
              console.log(
                chalk.red(
                  `Ошибка при покупке апгрейда: ${JSON.stringify(
                    buyResult,
                    null,
                    2
                  )}`
            ...
            console.error(
              chalk.red(`Ошибка при покупке апгрейда: ${error}`)
            )
            affordableUpgrades.shift() // Удаляем апгрейд из списка доступных
            cooldown = 10 * 1000 // 10 секунд cooldown при ошибке
            backoffMultiplier *= 2 // Увеличиваем backoff multiplier
          ...
          if (nearestUpgrade) {
            const { hoursToBuy, minutesToBuy, secondsLeft } =
              calculateTimeToBuy(clickerUser, nearestUpgrade)

            console.log(
              `Ближайший доступный апгрейд: ${
                nearestUpgrade.section
              }: ` +
              `${chalk.blue(nearestUpgrade.name)} ${chalk.yellow(
                nearestUpgrade.price.toLocaleString()
              )} ` +
              `- окупаемость: ${
            ...
            await new Promise((resolve) =>
              setTimeout(resolve, secondsLeft * 1000)
            )
